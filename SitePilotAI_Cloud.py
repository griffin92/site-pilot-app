...
try:
    ...
    st.error("🚨 BACKEND ACCESS DENIED: Ensure GEMINI_API_KEY is configured in Streamlit Cloud Secrets.")
    st.stop()
...
def get_pdf_info(file_bytes):
    """Extracts total page count from PDF."""
    return pdfinfo_from_bytes(file_bytes)["Pages"]

@st.cache_data
def convert_single_page(file_bytes, page_num):
    """Converts a specific PDF page to an image for AI processing."""
...
def create_pdf_report(project_name, content, title):
    """Bulletproof PDF export engine with margin crash prevention."""
    ...
    def safe_write(text):
        ...
        # Prevent unbreakable strings from crashing the PDF margin
...
def run_ai_with_progress(file_bytes, target_pages, prompt_text, success_message="Task Complete!"):
    """Handles visual loading bar and API call to Gemini."""
...
with st.sidebar:
    ...
    st.caption("Enterprise OS v20.0")
    st.divider()
...
if uploaded_file:
    ...
    # Check if new file uploaded, load its specific history
    if st.session_state.current_file != uploaded_file.name:
        ...
        # Load all history archives
        history_keys = ['audit', 'takeoff', 'schedule', 'est', 'intel', 'submittal']
        ...
        with c_tools:
            ...
            if st.button("🚀 Run Clash Audit"):
                if target_docs:
                    ...
                    prompt = "Identify physical clashes or missing dimensions. Start each with 'ISSUE: '."
                    res = run_ai_with_progress(file_bytes, p_scan, prompt, "Audit Complete!")
            ...
            if st.session_state.audit_results:
                ...
                for issue in st.session_state.audit_results: 
                    st.write(f"🚩 {issue}")
            ...
            if st.button("📊 Material Takeoff"):
                if target_docs:
                    p_scan = [int([k for k, v in st.session_state.drawing_index.items() if v == d][0]) for d in target_docs]
                    prompt = "Perform detailed material takeoff. Output continuous lines starting with 'TAKEOFF: '."
                    res = run_ai_with_progress(file_bytes, p_scan, prompt, "Takeoff Complete!")
            ...
            if st.session_state.takeoff_results:
                ...
                for item in st.session_state.takeoff_results: 
                    st.write(f"📦 {item}")
                ...
                for i, e in enumerate(st.session_state.audit_history):
                    with st.popover(f"🕒 {e['time']} Audit"):
                        for item in e['results']: 
                            st.write(f"- {item}")
                        pdf_bytes = create_pdf_report(uploaded_file.name, e['results'], "Audit")
                        st.download_button("📥 Download PDF", pdf_bytes, f"Audit_{i}.pdf", key=f"dla_{i}")
            ...
            with ha2:
                ...
                for i, e in enumerate(st.session_state.takeoff_history):
                    with st.popover(f"🕒 {e['time']} Takeoff"):
                        for item in e['results']: 
                            st.write(f"- {item}")
                        pdf_bytes = create_pdf_report(uploaded_file.name, e['results'], "Takeoff")
                        st.download_button("📥 Download PDF", pdf_bytes, f"Takeoff_{i}.pdf", key=f"dlt_{i}")
            ...
            with ha3:
                ...
                for i, e in enumerate(st.session_state.schedule_history):
                    with st.popover(f"🕒 {e['time']} Timeline"):
                        ...
                        pdf_bytes = create_pdf_report(uploaded_file.name, e['results'], "Timeline")
                        st.download_button("📥 Download PDF", pdf_bytes, f"Timeline_{i}.pdf", key=f"dls_{i}")
    ...
    with tab_est:
        ...
        with col_est:
            ...
            if st.button("🧮 Generate Baseline Estimate"):
                if target_docs:
                    ...
                    st.warning("Please return to the VDC tab and select target sheets.")
            ...
            if st.session_state.est_results:
                ...
                pdf_bytes = create_pdf_report(uploaded_file.name, st.session_state.est_results, "Estimate")
                st.download_button("📥 Export Estimate PDF", pdf_bytes, "Estimate.pdf", "application/pdf")
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        with col_doc:
            ...
            st.caption("Summarize Contracts, RFIs, or Geotech Reports uploaded in Slot 3.")
            ...
            if doc_file:
                ...
                if st.session_state.doc_intel_results: 
                    ...
                    pdf_bytes = create_pdf_report(uploaded_file.name, st.session_state.doc_intel_results, "Doc Summary")
                    st.download_button("📥 Export Summary PDF", pdf_bytes, "Summary.pdf", "application/pdf")
            else:
            ...
            with ea1:
                ...
                for i, e in enumerate(st.session_state.est_history):
                    with st.popover(f"🕒 {e['time']} | {e['desc']}"):
                        st.markdown(e['results'])
                        pdf_bytes = create_pdf_report(uploaded_file.name, e['results'], "Estimate")
                        st.download_button("📥 PDF", pdf_bytes, f"Est_{i}.pdf", key=f"dle_{i}")
            with ea2:
                st.markdown("**Document Summaries**")
                for i, e in enumerate(st.session_state.intel_history):
                    with st.popover(f"🕒 {e['time']} | {e['desc']}"):
                        st.markdown(e['results'])
                        pdf_bytes = create_pdf_report(uploaded_file.name, e['results'], "Summary")
                        st.download_button("📥 PDF", pdf_bytes, f"Doc_{i}.pdf", key=f"dli_{i}")
    ...
    with tab_admin:
        ...
        st.caption("Deep-scan Project Manuals (Slot 2) to automatically generate comprehensive Submittal registers.")
        ...
        if spec_file:
            ...
            if st.session_state.submittal_results:
                ...
                for s in st.session_state.submittal_results: 
                    st.write(f"📁 {s}")
                st.markdown('</div>', unsafe_allow_html=True)
                pdf_bytes = create_pdf_report(uploaded_file.name, st.session_state.submittal_results, "Submittal Register")
                st.download_button("📥 Export Submittal Log PDF", pdf_bytes, "Submittals.pdf", "application/pdf")
        else:
        ...
        with st.expander("🗄️ Admin Archives"):
            ...
            for i, e in enumerate(st.session_state.submittal_history):
                with st.popover(f"🕒 {e['time']} Register"):
                    for item in e['results']: 
                        st.write(f"- {item}")
                    pdf_bytes = create_pdf_report(uploaded_file.name, e['results'], "Submittal Log")
                    st.download_button("📥 PDF", pdf_bytes, f"Sub_{i}.pdf", key=f"dls_{i}")

# ==========================================
# 8. LANDING PAGE (NO FILES UPLOADED)
# ==========================================
else:
